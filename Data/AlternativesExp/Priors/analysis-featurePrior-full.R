...
###########################################################
library(reshape)
fp.set <- read.csv("long-full-120.csv")
...
####################
priors.full.wide <- reshape(priors.full, v.names="normalizedProb", direction="wide", 
                               timevar="featureSetNum", idvar=c("categoryID", "alternative", "featureNum"), drop=c("N", "sd", "se", "ci",
                                                                               "f1", "f2", "f3"))

write.csv(priors.full.wide, "featurePriors-set-wide-full.csv", row.names=FALSE)
