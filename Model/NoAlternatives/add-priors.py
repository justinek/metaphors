...
for i in range(len(altDict)):
    catID = str(i + 1)
    writeF = open("WithPriors/ID-" + catID + ".church", "w")
    animals = []
    featurePriors = []
    ...
    writeF.write("(define categories (list '" + " '".join(animals) + "))\n")
    writeF.write("(define utterances (list '" + " '".join(animals) + "))\n")
    writeF.write("(define (utterance-prior) (multinomial utterances '(" + " ".join(utterancePriors) + ")))\n")
