...
# For each target utterance, find only core features
...
for alpha in alphas:
    ...
    for l in targetF:
        if firstline == 0:
            ...
            features = coreFeaturesDict[utterance]
            writeF = open("CoreAnimals/" + utterance + "_" + str(numBins) + "_" + str(alpha) + "_noAltUtts.church", "w")
            ...
            writeF.write("(write-csv (append (list (list 'category '" + " '".join(features) + " 'prob)) (map flatten (zip (first interpretation) (second interpretation)))) '/Users/justinek/Dropbox/Work/Grad_school/Research/Metaphor/metaphors/Model/SeedingModel/CoreAnimalsOutput/" + utterance + "_" + str(numBins) + "_" + str(alpha) + "_noAltUtts.csv)")
    targetF.close()        
