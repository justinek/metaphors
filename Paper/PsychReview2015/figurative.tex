...
It has been observed consistently in the literature that ironic statements that are literally positive but express negative opinions (e.g. saying ``You're a great friend'' ironically to a friend who betrayed you) occur more frequently and are easier to comprehend than statements that are literally negative but express positive opinions (e.g. saying ``You're a terrible friend'' ironically to a friend who helped you). This phenomenon, termed the \emph{asymmetry of affect}, 
...
One potential source of error is the fact that most of the features in this dataset are gradable adjectives (e.g. \emph{strong}, \emph{large}, \emph{soft}) whose meanings vary with context and the types of entities they predicate. For example, a strong ant is probably much less strong than a strong person. In addition, our judgment of whether an ant is \emph{strong} depends on the class of entities that we compare it to---e.g. all other ants, all living things of its size, or all living things in general. As a result, an ant can be \emph{strong} compared to other living things of its size, but will be much less strong than an average person. In our experiments and model, we did not consider the context-sensitivity of these gradable features. In Experiment 2b, participants rated an ant as being strong with probability $0.69$ on average, but rated a person as being strong with probability $0.54$, presumably because they are judging \emph{strong} with respect to a class of insects in the \emph{ant} case, and with respect to a class of human beings in the \emph{person} case. In the qRSA model, a pragmatic listener using these priors will interpret ``Cam is an ant'' to mean that Cam is more likely than an average person to be strong, because an ant in more likely than a person to be strong. However, the intuitive interpretation of the metaphor is that Cam is less strong than an average person, presumably because people have the encyclopedic knowledge that when the comparison class includes human beings, an ant is very unlikely to be considered \emph{strong}. By using context-sensitive gradable adjectives as features without providing the model with correct context-sensitive feature probabilities, we prevent the from producing the appropriate interpretation for metaphors such as ``Cam is an ant.'' 
...
Another related source of error is the set of alternative utterances that the model considers. In the model simulations presented here, we only considered two syntactically similar utterances that a speaker could choose to say: ``Cam is a [\emph{animal}, e.g. whale]'', and ``Cam is a person.'' Because the 
...
Conventionality
Some metaphors are conventionalized, e.g. ``John is a fox'' means that John is attractive in a certain way that could be described as ``pretty.'' Even though the prior probability of a man being ``pretty'' is not high, the conventionalized metaphorical meaning of ``fox'' carries that connotation.

Nonliteral features
Some features may themselves have nonliteral interpretations. For example, it is highly unlikely for a person to literally ``quack'' like a duck quacks. However, a person could reasonably make noises or have a voice that resembles quacking in its loudness or annoyingness. While humans have the flexibility of interpreting the features metaphorically and assign high probability to the \emph{quacking} feature given the utterance ``John is a duck,'' the model is influenced by the low prior probability of a person literally being \emph{quacking} and ends up with still a low inferred probability of John having that feature. This raises the interesting question of how nonliteral interpretations of features are derived to begin with. Emergent features?
...
In the next section, we will begin to address some of these remaining issues (specifically, the issue of feature calibration and nonliteral features) using the case study of hyperbolic metaphors and relational metaphors.
