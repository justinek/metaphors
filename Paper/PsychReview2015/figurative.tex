...
\caption{Model's and participants' inferences about the weather state (x-axis) given a weather context (column) and an utterance (row). Each panel represents interpretations of an utterance in a weather context. The dark lines are participants' ratings; the light lines are model's posterior distributions over weather states.}
\label{model-state}
\end{figure}

\begin{figure}[t]
\scalebox{0.5}{\includegraphics{Plots/irony-affect-human-model-withpics.pdf}}
\caption{Model's and participants' inferences about the probability of valence and arousal (row) given a weather context (column) and an utterance (x-axis). The dark lines are participants' ratings; the light lines are model's posterior probabilities for valence and arousal given an utterance and weather context. The dotted lines are prior probabilities of valence and arousal for each weather context. Error bars are $95\%$ confidence intervals on the participants' ratings.}
\label{model-state}
\end{figure}
