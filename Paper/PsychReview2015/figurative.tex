...
\abstract{People use language to convey information that goes far beyond an utterance's literal meaning. In particular, figurative language such as hyperbole, irony, and metaphor showcases people's ability to incorporate background knowledge and social reasoning to infer relevant and true information from utterances that are false under their literal semantics. 
...
Although figurative language is prevalent in human communication, how people arrive at appropriate interpretations remains unclear.
...
Second, we can visually represent the weather to participants with minimal linguistic description in order to obtain measures of nonlinguistic contextual knowledge, for example, showing participants a picture of blue skies and asking them to judge how likely it is that someone would perceive the weather to be amazing or terrible. Finally, given the critical role that context plays in understanding irony, we can vary the context (a picture of gray, cloudy skies instead of blue skies) to observe how the same utterance is interpreted differently given different contextual knowledge. 
%This offers to our knowledge the first fine-grained manipulation and quantitative measure of context in studies of irony. 

In what follows, we first explore how an enriched space of affect affects the qRSA model and find that it produces ironic interpretations in a simple simulation. We then present two behavioral experiments that examine people's interpretations of utterances given different weather contexts. We show that by accounting for two types of affective dimensions---valence and arousal---our model produces interpretations that closely match humans'. 
...
\begin{figure}
\centering
\scalebox{0.6}{\includegraphics{Plots/sim12.pdf}}
...
The model is capable of non-literal interpretation: it produces a hyperbolic interpretation (that the weather is merely \textit{bad}) given ``The weather is terrible'' in the bad weather situation. However, it produces a literal interpretation (that the weather is \textit{terrible}) in the amazing weather situation. Since a pragmatic listener that only considers emotional valence does not believe that the speaker has any reason to choose a negative utterance to express positive affect (because the utterance communicates no true information), a model that only considers emotional valence is unlikely to infer a positive world state from a negative utterance (and vice versa), thus failing to evidence verbal irony. 
...
This reveals a critical puzzle in the interpretation of verbal irony. What true information \emph{could} a speaker communicate about a positive world state using a negative utterance? Affective science identifies two dimensions, termed valence and arousal, that underly the slew of emotions people experience \cite{russell1980circumplex}. 
...
\end{figure}
    %\begin{minipage}{0.45\textwidth}
       % \centering
  \begin{figure}
        \includegraphics[width=350pt, height=250pt]{Plots/priors.pdf}
...
\begin{figure}
        \includegraphics[width=320pt, height=280pt]{Plots/biplot.pdf}
...
\begin{figure}
    %\begin{minipage}{0.45\textwidth}
       \includegraphics[width=350pt, height=220pt]{Plots/affect-prior.pdf}
...
\begin{figure}[t]
\scalebox{0.5}{\includegraphics{Plots/model-state.pdf}}
...
Here we describe an experiment that elicits people's interpretations of utterances, which we then use to evaluate model predictions. 
...
\includegraphics[width=340pt, height=170pt]{Plots/scatter_irony.pdf}
...
\subsubsection{Model}
We again introduce a literal listener $L_0$, who interprets the utterances as meaning that Cam is literally a shark. Since $L_0$ believes Cam is a shark, she also believes that Bob is likely to have features associated with sharks, for example, being scary or fierce.
...
For simplicity, in this model we restrict the number of possible categories to which a member may belong to $c_a$ and $c_p$, denoting an animal category or a person category, respectively. We also restrict the possible features of Cam under consideration to a vector of size three: $\vec f = [f_1, f_2, f_3]$, where $f_i$ is either $0$ or $1$%(for example, the three features could be scary, sleek, and finned)
.\footnote{In principle the model can be extended to accommodate more categories and features.}
...
Analogy? three examples of more general types of metaphor that our model may or may not handle

Verbs: The ice skater flew across the rink.
Abstract-abstract: Some brains are warehouses.
Analogy: 
...
The work we described invites many directions for future research. %that can be pursued using both our modeling framework and experimental paradigm.
